...
class Statistician(nn.Module):
    ...
    def _sample_helper_(self, c, batch_size, sample_size, z_stochastic=True):
        # latent decoders
        pz_samples = []
        z = None
        for i, latent_decoder in enumerate(self.latent_decoders):
            z_mean, z_logvar = latent_decoder(z, c)
            if i == 0:
                z_mean = z_mean.repeat(sample_size, 1)
                z_logvar = z_logvar.repeat(sample_size, 1)
            if z_stochastic:
                z = self.reparameterize_gaussian(z_mean, z_logvar).contiguous()
            else:
                z = z_mean
            pz_samples.append(z)
        ...
        out = x_mean.view(batch_size, sample_size, self.n_features)
        return out


    def sample_unconditioned(self, sample_size):
        # TODO : is it the same?
        c = torch.rand(self.batch_size, self.c_dim).cuda()
        return self._sample_helper_(c, self.batch_size, sample_size)


    def sample_conditioned(self, inputs):
        c, _ = self.statistic_network(inputs)
        return self._sample_helper_(c, self.batch_size, self.sample_size)

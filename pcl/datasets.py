...
class Uniform15KPC(Dataset):
    ...
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
            ...
            normalize_per_shape=False, normalize_per_dataset=False, random_subsample=False,
            normalize_std_per_axis=False,
            all_points_mean=None, all_points_std=None,
            uniform_split=False, split_path=None,
    ):
        ...
        if uniform_split:
            assert split_path is not None, "Requires split_path"
            all_synsetid_mids = np.load(split_path).item()
        else:
            all_synsetid_mids = None

        self.all_cate_mids = []
        self.cate_idx_lst = []
        self.all_points = []
        # self.mesh_files = []
        # for cate_idx, subd in tqdm(enumerate(self.subdirs), desc='Subdirectories', leave=True):
        for cate_idx, subd in enumerate(self.subdirs):
            print("CateIDX:%s"%cate_idx)
            ...
            if uniform_split and all_synsetid_mids is not None:
                # all_mids = np.load(os.path.join(root_dir, subd, "%s.npy"%self.split))
                all_mids = all_synsetid_mids[subd][self.split]
            else:
                all_mids = []
        ...
        # self.mesh_files = self.mesh_files[]

        # Normalization
        self.all_points = np.concatenate(self.all_points) # (N, 15000, 3)
        assert not (normalize_per_dataset and normalize_per_shape), "Can normalize per shape and dataset together"
        self.normalize = normalize_per_shape or normalize_per_dataset
        ...
        if all_points_mean is not None and all_points_std is not None:
            assert normalize_per_dataset, "Only normalize per dataset allows swapping normalization stats"
            self.all_points_mean = all_points_mean
            self.all_points_std = all_points_std
        elif self.normalize:
            # self.all_points has shape (B, N, 3)
            if self.normalize_per_shape:
                B, N = self.all_points.shape[:2]
                self.all_points_mean = self.all_points.mean(axis=1).reshape(B, 1, 3)
                if normalize_std_per_axis:
                    self.all_points_std = self.all_points.reshape(B, N, -1).std(axis=1).reshape(B, 1, 3)
                else:
                    self.all_points_std = self.all_points.reshape(B, -1).std(axis=1).reshape(B, 1, 1)
            elif self.normalize_per_dataset:
            ...
            self.all_points_std  = np.ones((1, 1, 1))
    ...
    def get_pc_stats(self, idx):
        if self.normalize_per_shape:
            m = self.all_points_mean[idx].reshape(1,3)
    ...
    def __getitem__(self, idx):
        ...
        # return idx, tr_out, te_out, m, s, cate_idx
        return {
            'idx' : idx,
            'train_points' : tr_out,
            'test_points' : te_out,
...
class ShapeNet15kPointClouds(Uniform15KPC):
    ...
    def __init__(self, root_dir="data/ShapeNetCore.v2.PC15k",
        ...
        super(ShapeNet15kPointClouds, self).__init__(
            ...
            split_path="splits/shapenet_split.npy" # the file that contains the uniform splits
        )
...
def _get_MN40_datasets_(args, data_dir=None):
    tr_dataset = ModelNet40PointClouds(
            ...
            normalize_per_dataset=args.normalize_per_dataset,
    ...
    te_dataset = ModelNet40PointClouds(
            ...
            normalize_std_per_axis=args.normalize_std_per_axis,
            normalize_per_dataset=args.normalize_per_dataset,
            all_points_mean=(tr_dataset.all_points_mean if args.normalize_per_dataset else None),
            all_points_std=(tr_dataset.all_points_std if args.normalize_per_dataset else None),
    )
...
def get_datasets(args):
    if args.dataset_type == 'shapenet15k':
        tr_dataset = ShapeNet15kPointClouds(
                ...
                normalize_std_per_axis=args.normalize_std_per_axis,
                random_subsample=True,
                uniform_split=args.shapenet_uniform_split)
